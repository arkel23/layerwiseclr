...
class SupConLossDebug(nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf.
    It also supports the unsupervised contrastive loss in SimCLR"""
    def __init__(self, temperature=0.07, contrast_mode='all',
                 base_temperature=0.07):
        super(SupConLossDebug, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        print('masks and labels shapes: ', mask.shape, mask, labels) # labels.shape
        ...
        print('contrast_count, contrast_feature, anchor_feature, anchor_count: ', 
              contrast_count, contrast_feature.shape, anchor_feature.shape, anchor_count, 
        ...
        print('anchor_dot_contrast, logits_max, logits: ', 
              anchor_dot_contrast.shape, logits_max.shape, logits.shape, anchor_dot_contrast, logits_max, logits)
        ...
        print('mask, logits_mask, exp_logits, log_prob, mean_log_prob_pos: ',
              mask.shape, logits_mask.shape, exp_logits.shape, log_prob.shape, mean_log_prob_pos,
              mask, logits_mask, exp_logits, log_prob)
